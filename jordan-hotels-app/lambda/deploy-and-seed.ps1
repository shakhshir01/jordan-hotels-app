...
# Seed hotels from Xotelo (Jordan) by default.
# Override any of these by setting env vars before running this script.
if (-not $env:SEED_FROM_XOTELO) { $env:SEED_FROM_XOTELO = "1" }
if (-not $env:XOTELO_LOCATION_KEY) { $env:XOTELO_LOCATION_KEY = "g293985" }
if (-not $env:XOTELO_MAX_HOTELS) { $env:XOTELO_MAX_HOTELS = "2500" }
if (-not $env:XOTELO_PAGE_LIMIT) { $env:XOTELO_PAGE_LIMIT = "100" }
if (-not $env:XOTELO_SLEEP_MS) { $env:XOTELO_SLEEP_MS = "140" }
